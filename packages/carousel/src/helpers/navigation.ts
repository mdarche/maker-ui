...
export function updateNodes(
  oldItems: SlideItem[],
  newItems: SlideItem[],
  prevChildren: SlideItem[] | undefined,
  slide: number,
  infinite: boolean
): SlideItem[] {
  ...
  const matchedItems = oldItems.map((oldItem) => {
    return newItems.find((newItem) => oldItem.key === newItem.key) as SlideItem
  })
...
export function areItemsNotMatched(items: SlideItem[]): boolean {
