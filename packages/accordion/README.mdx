...
When implemented alongside the `use client` directive, the Accordion can be accessed as a compound component. This offers a more declarative API and enhances readability:
...
The Accordion wrapper acts as a context provider that stores shared settings for all Accordion panels. It extends all `HTMLDivElement` props and also includes:
...
| `--accordion-btn-bg`            | `button.background`       | Button background             |
...
| `--accordion-btn-bg-active`     | `button.backgroundActive` | Button background when active |
...
| `--accordion-panel-bg`          | `panel.background`        | Panel background              |
...
| `panelGroup`   | The outermost wrapper for the `Accordion.Panel` component.                      |
