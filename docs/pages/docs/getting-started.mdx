...
To begin using Maker UI in your project, install the `maker-ui` library from npm:
...
<Callout type="alert">
  If you just want to use Primitives, you can simply import{' '}
  <code>@maker-ui/primitives</code>
</Callout>
...
The JSX from above outputs a semantically structured DOM tree. This example would create a basic single-column page layout that supports mobile navigation right out of the box.
...
The real magic behind Maker UI's layout system is how it automatically supports responsive scaling and accessibility. Let's create a `MakerUIOptions` object that will tell our header how it should look and feel on desktop vs mobile:
...
With the options configuration object, you can control 50+ settings that tell your layout how to behave. Use `MakerUIOptions` to set color modes, handle error boundaries, configure dropdown menus, and more.
...
Learn about menus in the [Navbar docs](/docs/navbar).

## Add Custom Styles

It's time to add some styles to our responsive layout skeleton.

All Maker UI layout components accept the `css` and `breakpoints` props ([covered here](/docs/styling-components)). However, adding too many styles directly to the layout tree can quickly turn our file into a mess of CSS-in-JS.

To address this, Maker UI's root `<Layout>` component accepts a `styles` prop where we can add global styles that are applied to the entire document.
...
<Callout>
  ...
  <ul>
    ...
    <li>Layout API component styles via ID or class selector</li>
  </ul>
</Callout>
...
<Callout>
  ...
  <a href="/docs/layout-api">Layout API docs</a>, of course) is to open DevTools
  and identify the proper ID or class selector for the job.
</Callout>
