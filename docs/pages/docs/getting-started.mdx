...
Maker UI includes a set of layout components ([Layout API](/docs/layout-api)) as well as basic elements ([Primitives](/docs/primitives)) that you can use as building blocks in your page designs.

<Callout type="alert">
If you do not want to use the complete Maker UI layout system, you can simply install <span>@maker-ui/primitives</span> and import its components as needed.
</Callout>

## Create a Basic Layout

Let's create a basic layout with the Layout API. In a new component file, add the following:
...
The real magic behind Maker UI is how it supports automatic responsive scaling and accessibility. Let's create an `options` object that will tell our header how it should look and feel on desktop vs mobile:
...
When your new layout is all ready to go, import it into your app's root component and treat it like a top-level application wrapper. 
...
Also note that this Getting Started demo doesn't even begin to touch on all of the amazing features and functions of Maker UI's layout system.
