...
Under the hood, all Maker UI components use a custom JSX factory powered by Emotion to scan for the `breakpoints` and `css` props. These custom props let you specify your media query breakpoints as an array of values that correspond with CSS rules. 

For more details on the components you can use in your project, see Primitives, Layout API, and Component API. 
