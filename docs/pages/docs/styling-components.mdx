...
All Maker UI components and primitives have the special `breakpoints` and `css` props. These props let you specify media query breakpoints as an array of values that correspond with CSS rules.
...
<Callout>
  Maker UI is built with TypeScript, so your text editor may even try to help
  you autocomplete CSS values.
</Callout>
...
<Callout>
  This is the recommended way to handle complex component styles because it
  generates fewer style tags in your document head and will make your component
  easier to maintain.
</Callout>
