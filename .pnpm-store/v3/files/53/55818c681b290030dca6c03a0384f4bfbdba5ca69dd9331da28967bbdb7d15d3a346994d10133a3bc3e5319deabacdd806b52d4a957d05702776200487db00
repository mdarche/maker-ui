...
var loadJson = async (filepath) => {
  try {
    return _chunkBYILCV74js.jsoncParse.call(void 0, await _fs2.default.promises.readFile(filepath, "utf8"));
  } catch (error) {
...
var colorize = (type, data, onlyImportant = false) => {
  ...
  return _chunkJHDYFHQIjs.colorette_exports[color](data);
};
var makeLabel = (name, input, type) => {
  return [
    name && `${_chunkJHDYFHQIjs.dim.call(void 0, "[")}${name.toUpperCase()}${_chunkJHDYFHQIjs.dim.call(void 0, "]")}`,
...
var reportSize = (logger, format, files) => {
  ...
  for (const name of filenames) {
    logger.success(format, `${_chunkJHDYFHQIjs.bold.call(void 0, padRight(name, maxLength))}${_chunkJHDYFHQIjs.green.call(void 0, prettyBytes(files[name]))}`);
  }
};
